...
import io.frama.parisni.spark.sync.copy.DeltaToDeltaYaml.Database
...
class DeltaToDeltaTest extends FunSuite with SparkSessionTestWrapper{

  //@Test
  def testDelta2Delta(): Unit = {
    //println("io.frama.parisni.spark.sync.Sync Delta2Delta")
    /*val deltaConfTest:io.frama.parisni.spark.sync.copy.DeltaConfTest = new io.frama.parisni.spark.sync.copy.DeltaConfTest
    ...
    println("delta2Delta ------------------")
    val d2d2:DeltaToDelta2 = new DeltaToDelta2
    d2d2.sync(spark, palette)
