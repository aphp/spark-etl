...
object SolrCloudUtil extends LazyLogging {
  ...
  def deleteCollection(
    ...
    try {
      ...
      case e: Exception =>
        logger.error(
          "Failed to delete collection " + collectionName + " due to: " + e
        )
    }
  }
  ...
  def createCollection(
    ...
    if (confDir != null) {
      assertTrue(
        "Specified Solr config directory '" + confDir.getAbsolutePath + "' not found!",
        confDir.isDirectory
      )
      // upload test configs
  ...
  def ensureAllReplicasAreActive(
      collectionName: String,
      numShards: Int,
      replicationFactor: Int,
      maxWaitSecs: Int,
    ...
    val slices: java.util.Collection[Slice] =
      cs.getCollection(collectionName).getActiveSlices
    ...
    while (waitMs < maxWaitMs && !allReplicasUp) {
      ...
      for (shard: Slice <- cs.getCollection(collectionName).getActiveSlices) {
        ...
        logger.info(
          "Found " + replicas
            .size() + " replicas and leader on " + leader.getNodeName + " for " + shardId + " in " + collectionName
        )
        ...
        for (replica: Replica <- replicas) {
          ...
          if (!"active".equals(replicaState)) {
            logger.info(
              "Replica " + replica.getName + " for shard " + shardId + " is currently " + replicaState
            )
            allReplicasUp = false
          }
        }
    ...
    if (!allReplicasUp)
      fail(
        "Didn't see all replicas for " + collectionName +
    ...
    logger.info(
      "Took '" + diffMs + "' ms to see all replicas become active for " + collectionName
    )
  }

  def printClusterStateInfo(
      collectionName: String,
      cloudClient: CloudSolrClient
  ): String = {
    cloudClient.getZkStateReader.updateLiveNodes()
  ...
  def dumpSolrCollection(
    ...
    logger.info(
      "Found " + qr.getResults.getNumFound + " docs in " + collectionName
    )
    var i = 0
  ...
  def buildCollection(
    ...
    for (n: Int <- 0 to numDocs - 1) {
      inputDocs.update(
        n,
        collection + "-" + n + ",foo" + n + ",bar" + n + "," + n + ",[a;b],[1;2]"
      )
    }
  ...
  def buildCollection(
    ...
    if (inputDocs != null) {
      val numDocsIndexed: Int =
        indexDocs(zkHost, collection, inputDocs, cloudClient)
      Thread.sleep(1000L)
    ...
    inputDocs.foreach(row => {
      ...
      if (fields.length < 6)
        throw new IllegalArgumentException(
          "Each test input doc should have 6 fields! invalid doc: " + row
        )
