...
class ConstraintsTest extends QueryTest with SparkSessionTestWrapper {
  ...
  assume(
    !Constraints.skipped,
    "Constraint checking is skipped by environment variable"
  )
  ...
  test("Check success") {
    ...
    val check = Constraints.fromSchema(
      Schema(
        Seq(
          Field(
            "str",
  ...
  test("Check failures") {
    ...
    val checkNull = Constraints.fromSchema(
      Schema(
        Seq(
    ...
    val checkNan = Constraints.fromSchema(
      Schema(
        Seq(
  ...
  test("Check ratio") {
    ...
    val checkHalf = Constraints.fromSchema(
      Schema(
        Seq(
  ...
  test("To Yaml") {
    ...
    assertResult(
      ...
      Constraints
        .schema2yaml(Schema(Seq(verbose)))
        .lines
        .flatMap {
