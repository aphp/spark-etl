...
case class DataFrameQuery(
    df: DataFrame,
    as: String,
    override val joinAs: String,
    override val idFields: Seq[String] = List("id")
) extends Query

object DataFrameQuery {
  def apply(df: DataFrame, as: String): DataFrameQuery =
    DataFrameQuery(df, as, as)
}

case class DataFrameEventQuery(
    df: DataFrame,
    as: String,
    eventCol: String,
    startsCol: String,
