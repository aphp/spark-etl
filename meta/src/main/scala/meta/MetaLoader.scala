...
class MetaLoader(host: String, database: String, schema: String, user: String) extends FeatureExtract with LazyLogging {
  ...
  def loadSchema(df: DataFrame, dbName: String): Unit = {
    val opt = Map("joinColumns" -> "lib_database", "pgJoinSql" ->
      "select ids_database, lib_database from meta_database")
  ...
  def loadTable(df: DataFrame, dbName: String): Unit = {
    val opt = Map("joinColumns" -> "lib_database,lib_schema", "pgJoinSql" ->
      "select ids_schema, d.lib_database, s.lib_schema from meta_schema s join meta_database d using (ids_database)")
    val result = getLookup(df, opt)
  ...
  def loadColumn(df: DataFrame, dbName: String): Unit = {
    val opt = Map("joinColumns" -> "lib_database,lib_schema,lib_table", "pgJoinSql" ->
      "select ids_table, d.lib_database, s.lib_schema, t.lib_table from meta_table t join meta_schema s using (ids_schema) join meta_database d using (ids_database)")
    val result = getLookup(df, opt)
  ...
  def loadReference(df: DataFrame, dbName: String): Unit = {
    val optSource = Map("joinColumns" -> "lib_database_source,lib_schema_source,lib_table_source,lib_column_source", "pgJoinSql" ->
    ...
    val optTarget = Map("joinColumns" -> "lib_database_target,lib_schema_target,lib_table_target,lib_column_target", "pgJoinSql" ->
      "select ids_column as ids_target, d.lib_database as lib_database_target, s.lib_schema as lib_schema_target, t.lib_table as lib_table_target, c.lib_column as lib_column_target from meta_column c join meta_table t using (ids_table) join meta_schema s using (ids_schema) join meta_database d using (ids_database)")
