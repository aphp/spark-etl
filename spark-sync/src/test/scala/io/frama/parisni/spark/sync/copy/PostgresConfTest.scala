...
class PostgresConfTest extends QueryTest with SparkSessionTestWrapper {
  ...
  def createPostgresTables(): Unit = { // Uses JUnit-style assertions
    ...
    val sInputDF: DataFrame = ((
      1,
      "id1s",
      "test details of 1st row source",
      ...
      (
        2,
        "id2s",
        "test details of 2nd row source",
      ...
      (
        3,
        "id3s",
        "test details of 3rd row source",
      ...
      (
        4,
        "id4s",
        "test details of 4th row source",
      ...
      (
        5,
        "id5",
        "test details of 5th row source",
      ...
      (
        6,
        "id6",
        "test details of 6th row source",
    ...
    val sOutputDF: DataFrame = spark.read
      .format("postgres")
      .option("url", url)
    ...
    println(
      "Table source exists = " + checkTableExists(
        spark,
        url,
        "source",
        "public"
    ...
    println(
      "Table target exists = " + checkTableExists(
        spark,
        url,
        "target",
        "public"
  ...
  def calculateMaxDate(): Unit = {
    ...
    val mapy = Map(
      ...
      "S_DATE_FIELD" -> "date_update",
      "T_TABLE_NAME" -> "target",
      "T_TABLE_TYPE" -> "postgres", //"T_DATE_MAX" -> "2010-10-16",
      "HOST" -> "localhost",
      "PORT" -> s"${pg.getEmbeddedPostgres.getPort}",
      "DATABASE" -> "postgres",
      "USER" -> "postgres",
  ...
  def sync(): Unit = {
    ...
    val portPg =
      pg.getEmbeddedPostgres.getPort.toString //database.portPg.toString
