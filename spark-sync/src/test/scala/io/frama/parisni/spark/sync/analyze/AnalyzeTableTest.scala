...
class AnalyzeTableTest extends QueryTest with SparkSessionTestWrapper {
  ...
  def analyzeTest(): Unit = {
    println(
      "Before io.frama.parisni.spark.sync.AnalyzeTableTest -----------------------"
    )
    //Create test tables
    createTables()
    ...
    println(
      "After io.frama.parisni.spark.sync.AnalyzeTableTest -----------------------"
    )
  }
  ...
  def createTables(): Unit = {
    ...
    val metaTableDF: DataFrame =
      ((1, "spark-prod", "t1", Timestamp.valueOf("2020-03-16 00:00:00"), -1) ::
        ...
        Nil)
        .toDF("ids_table", "lib_database", "lib_table", "last_analyze", "hash")
    metaTableDF.write
      .format("postgres")
      .option("url", url)
    ...
    val metaColumnDF: DataFrame = ((1, 1, "t1", "c1t1", -1) ::
      (2, 1, "t1", "c2t1", -1) ::
      (3, 2, "t2", "c1t2", -1) ::
      (4, 2, "t2", "c2t2", -1) ::
      (5, 3, "t3", "c1t3", -1) ::
    ...
    // Create table "t1"
    val t1DF: DataFrame = ((1, "text l1", "date l1") :: (
      2,
      "text l2",
      "date l2"
