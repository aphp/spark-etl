...
object Constraints extends LazyLogging with DefaultYamlProtocol {
  ...
  case class Field(
    ...
    pattern: String = "",       // Note: matching empty string will fail anyway, implemented with regexp_extract != ""
    minimum: Double = Double.NegativeInfinity,
    maximum: Double = Double.PositiveInfinity,  // Note: NaN fails maximum, https://github.com/awslabs/deequ/issues/224
    noNan: Boolean = false,
    allowed: Array[String] = Array.empty
  ) {
    val level: CheckLevel.Value = if (fatal) CheckLevel.Error else CheckLevel.Warning
    ...
    lazy val checks: Seq[Check] = Seq (
      if (required) Some(mkCheck("Required").hasCompleteness(name, assertion)) else None,
      if (unique) Some(mkCheck("Unique").hasUniqueness(name, assertion)) else None,
      if (!pattern.isEmpty) Some(mkCheck(s"/$pattern/").hasPattern(name, pattern.r, assertion)) else None,
      if (!minimum.isNegInfinity) Some(mkCheck(s">=$minimum").isGreaterThanOrEqualTo(name, minimum.toString, assertion)) else None,
      if (!maximum.isPosInfinity) Some(mkCheck(s">=$maximum").isLessThanOrEqualTo(name, maximum.toString, assertion)) else None,
      if (noNan) Some(mkCheck("No NaN").satisfies(s"not isnan(`$name`)", s"NotNaN($name)", assertion)) else None,
      if (allowed.nonEmpty) Some(mkCheck(allowed.mkString("In(", ", ", ")")).isContainedIn(name, allowed, assertion)) else None
    ).flatten
  }
  ...
  implicit object FieldFormat extends YamlFormat[Field] {
    override def read(yaml: YamlValue): Field = yaml.asYamlObject.getFields(YamlString("name")) match {
      case Seq(YamlString(name)) =>
        val fields = yaml.asYamlObject.fields
        def get[A](key: String, default: A) = fields.get(YamlString(key)).map {
            case a: A => a
        ...
        Field(name,
          ...
          minimum = get("minimum", YamlNumber(Double.NegativeInfinity)).convertTo[Double],
          maximum = get("maximum", YamlNumber(Double.PositiveInfinity)).convertTo[Double],
          noNan = get("noNan", YamlBoolean(false)).boolean,
          allowed = get("allowed", YamlArray(Vector.empty[YamlString])).elements.map {
            case YamlNull => ""
    ...
    override def write(field: Field): YamlValue = YamlObject(Seq(
      Some("name" -> YamlString(field.name)),
      if (field.ratio != 1.0d) Some("ratio" -> YamlNumber(field.ratio)) else None,
      ...
      if (!field.pattern.isEmpty) Some("pattern" -> YamlString(field.pattern)) else None,
      if (!field.minimum.isNegInfinity) Some("minimum" -> YamlNumber(field.minimum)) else None,
      if (!field.maximum.isPosInfinity) Some("maximum" -> YamlNumber(field.maximum)) else None,
      if (field.noNan) Some("noNan" -> YamlBoolean(true)) else None,
      if (field.allowed.nonEmpty) Some("allowed" ->
        YamlArray(field.allowed.map(YamlString(_).asInstanceOf[YamlValue]) :_*)) else None
    ).flatten.map{ case (k, v) => YamlString(k).asInstanceOf[YamlValue] -> v }.toMap)
  }
  ...
  def schema2yaml(schema: Schema): String = schemaFormat.write(schema).prettyPrint
  ...
  class ConstraintException(val result: VerificationResult)
    ...
    def failed(fatalOnly: Boolean = false): Iterator[CheckResult] = {
      val filter = if (fatalOnly) (_: CheckResult).status == CheckStatus.Error
  ...
  def apply(checks: Check *)(df: DataFrame): Try[DataFrame] = {
    ...
    if (loggable) {
      for ((check, ress) <- result.checkResults;
           res <- ress.constraintResults if res.status != ConstraintStatus.Success) {
        val message = s"${res.constraint} failed: ${res.message.getOrElse("Unknown failure")}"
