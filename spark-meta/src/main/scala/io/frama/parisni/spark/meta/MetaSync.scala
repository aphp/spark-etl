...
object MetaSync extends App with LazyLogging {
  ...
  val Array(yamlPath:String, logLevel:String) = args match {
    ...
    case Array(_) =>
      logger.info("No logLevel provided --> Default logLevel '%s' is applied".format(defaultLogLevel))
      Array(args(0), defaultLogLevel)

    case _ =>
      System.err.println("Expected arguments: args(0)=YamlPath\targs(1)=sparkLogLevel")
  ...
  private def getLogLevel(loglevel:String):String={
    ...
    if( Seq("ALL", "DEBUG", "ERROR", "FATAL", "INFO", "OFF", "TRACE", "WARN").contains(loglevel.trim.toUpperCase)) {
      loglevel.trim.toUpperCase
    }else{
      logger.warn("Loglevel %s is not recognized --> Default logLevel %s is then applied".format(loglevel,defaultLogLevel))
      defaultLogLevel
    }
  }
