import DeltaToSolr.{database, spark}
import DeltaToSolrYaml.Database
...
class DeltaToSolr2 extends App with LazyLogging {

  val filename = "deltaToSolr.yaml"     //args(0)
  val ymlTxt = Source.fromFile(filename).mkString
  val yaml = ymlTxt.stripMargin.parseYaml
  val database = yaml.convertTo[Database]
    ...
    try {
      ...
      for(table <- database.tables.getOrElse(Nil)) {
        ...
        val config = Map("S_TABLE_NAME" -> tableDelta, "S_TABLE_TYPE" -> "delta",
          ...
          "T_TABLE_NAME" -> tableSolr, "T_TABLE_TYPE" -> "solr",
          "ZKHOST" -> zkHost, "T_DATE_MAX" -> dateMax   //, "T_LOAD_TYPE" -> loadType
        )
