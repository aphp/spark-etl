...
class JoinTest extends QueryBaseTest {
  ...
  test("inner join") {
    ...
    assertQuery (messages, {
      peopleDf.join(messagesDf, peopleDf("person_id") === messagesDf("person_id"), "inner")
    }) {
      person + message
    }
  ...
  test("left outer join") {
    ...
    assertQuery(messages + 1, {
      peopleDf.join(messagesDf, peopleDf("person_id") === messagesDf("person_id"), "left_outer")
    }) {
      person % message
    }
  ...
  test("right outer join") {
    ...
    assertQuery(messages + 1, {
      peopleDf.join(messagesDf, peopleDf("person_id") === messagesDf("person_id"), "right_outer")
    }) {
      person %> message
    }
  ...
  test("full outer join") {
    ...
    assertQuery(messages + 2, {
      peopleDf.join(messagesDf, peopleDf("person_id") === messagesDf("person_id"), "full_outer")
    }) {
      person %% message
    }
    // Only Carlos didn't post any message
  ...
  test("left anti join") {
    // Only Carlos didn't post any message
    assertQuery(1, {
      peopleDf.join(messagesDf, peopleDf("person_id") === messagesDf("person_id"), "left_anti")
    }) {
      person - message
    }
  }
  ...
  test("left semi join") {
    ...
    assertQuery(people - 1, {
      peopleDf.join(messagesDf, peopleDf("person_id") === messagesDf("person_id"), "left_semi")
    }) {
      person ^ message
    }
  ...
  test("joiners") {
    ...
    assertQuery(messages,{
      ...
      (person + message).df
        ...
        .select(messagesDf.columns.map(messagesDf(_)) ++ peopleDf.columns.map(peopleDf(_)) :_*)
    }) {
    ...
    assertResult(InnerJoin(person, message, CommonColumnsJoiner))(person + ~message)
    assertQuery(messages) {
      // explicit column names
      person + message.on("person_id" /*, ... */)
    }
    assertResult(InnerJoin(person, message, "person_id" /*, ... */))(person + message.on("person_id" /*, ... */))
    ...
    assertQuery(5, {
      peopleDf
        .join(topicsDf, peopleDf("person_id") === topicsDf("author_id"), "inner")
        .join(messagesDf, (peopleDf("person_id") === messagesDf("person_id"))
                       && (topicsDf("topic_id") === messagesDf("topic_id")), "inner")
    }) {
      // left col -> right col
    ...
    assertQuery(5, {
      ...
      (topic + person.on("author_id" -> "person_id") + message).df
        // (column order swapped)
        .select(peopleDf.columns.map(peopleDf(_))
  ...
  test("self join") {
    ...
    assertQuery(2, {
      messagesDf.as("parent").join(messagesDf.as("message"),
        col("parent.message_id") === col("message.parent_id"), "inner")
    }) {
  ...
  test("select") {
    ...
    assertDF(5, {
      peopleDf.join(topicsDf, peopleDf("person_id") === topicsDf("author_id"), "inner")
              .join(messagesDf, (peopleDf("person_id") === messagesDf("person_id"))
                             && (topicsDf("topic_id") === messagesDf("topic_id")), "inner")
        .select(peopleDf.columns.map(c => peopleDf(c).as("person_" + c))
             ++ messagesDf.columns.map(c => messagesDf(c).as("message_" + c)) :_*)
    }) {
    ...
    assertDF(messages, {
      peopleDf.join(messagesDf, peopleDf("person_id") === messagesDf("person_id"), "inner")
        .select(peopleDf.columns.map(c => peopleDf(c).as("p_" + c))
  ...
  test("group by") {
    assertDF(people - 1, {
      peopleDf.join(messagesDf, Seq("person_id"), "inner")
        .groupBy(peopleDf.columns.map(peopleDf(_)) :_*).agg(countDistinct(messagesDf("message_id")))
    }) {
      // Carlos didn't post
      (person + message) / person
    }
    assertDF(people, {
      peopleDf.join(messagesDf, Seq("person_id"), "left_outer")
        .groupBy(peopleDf.columns.map(peopleDf(_)) :_*).agg(countDistinct(messagesDf("message_id")))
    }) {
      (person % message) / person
    }
    ...
    assertDF(people - 1, {
      peopleDf.join(messagesDf, Seq("person_id"), "inner")
        ...
        .agg(countDistinct(messagesDf("message_id")), countDistinct(messagesDf("topic_id")))
    }) {
