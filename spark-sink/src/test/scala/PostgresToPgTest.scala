...
class PostgresToPgTest extends SolrConfTest{    //FunSuite with SparkSessionTestWrapper
  ...
  def testPg2Pg(): Unit = {
    ...
    val pgc:PostgresConf = new PostgresConf(Map("T_LOAD_TYPE" -> "full", "S_TABLE_TYPE" -> "postgres",
            "T_TABLE_TYPE" -> "postgres"), List(""), List(""))
    pgc.writeSource(spark, s_inputDF, "localhost", pg.getEmbeddedPostgres.getPort.toString, "postgres",
      "postgres", "public", "source", "full", "hash")
