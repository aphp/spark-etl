...
class MetaLoader(
  ...
  def getLookup(df: DataFrame, options: Map[String, String]): DataFrame = {
    ...
    val joinTable = df.sparkSession.read
  ...
  def loadSchema(df: DataFrame, dbName: String): Unit = {
    ...
    writeScd1(
      active(result),
      "meta_schema",
      s"$LIB_DATABASE,$LIB_SCHEMA",
      s"$LIB_DATABASE = '$dbName' AND $LIB_SCHEMA ~* '${regexpFilter.getOrElse(".*")}'",
      "is_active = false"
    )
  }
  ...
  def loadTable(df: DataFrame, dbName: String): Unit = {
    val opt = Map(
      "joinColumns" -> s"$LIB_DATABASE,$LIB_SCHEMA",
    ...
    writeScd1(
      active(result),
      "meta_table",
      s"$LIB_DATABASE,$LIB_SCHEMA,$LIB_TABLE",
      s"$LIB_DATABASE = '$dbName' AND $LIB_SCHEMA ~* '${regexpFilter.getOrElse(".*")}'",
      "is_active = false"
    )
  }
  ...
  def loadColumn(df: DataFrame, dbName: String): Unit = {
    ...
    writeScd1(
      active(result),
      "meta_column",
      s"$LIB_DATABASE,$LIB_SCHEMA,$LIB_TABLE,lib_column",
      s"$LIB_DATABASE = '$dbName' AND $LIB_SCHEMA ~* '${regexpFilter.getOrElse(".*")}'",
      "is_active = false"
    )
  }
  ...
  def loadReference(df: DataFrame, dbName: String): Unit = {
    val optSource = Map(
      "joinColumns" -> "lib_database_source,lib_schema_source,lib_table_source,lib_column_source",
      "pgJoinSql" ->
    ...
    val optTarget = Map(
      "joinColumns" -> "lib_database_target,lib_schema_target,lib_table_target,lib_column_target",
    ...
    writeScd1(
      active(resultTarget),
      "meta_reference",
      "lib_database_source,lib_schema_source,lib_table_source,lib_column_source,lib_database_target,lib_schema_target,lib_table_target,lib_column_target,lib_reference",
      s"lib_database_source = '$dbName' AND ${LIB_SCHEMA}_source ~* '${regexpFilter
        .getOrElse(".*")}'",
      "is_active = false"
    )
  }
