...
trait TestSuiteBuilder
    extends SparkSolrFunSuite
    with SparkSolrContextBuilder
...
/** *
  ...
  * trait EventsimBuilder extends io.frama.parisni.spark.sync.copy.TestSuiteBuilder {
  *
  ...
  * }
  *
  * trait MovielensBuilder extends io.frama.parisni.spark.sync.copy.TestSuiteBuilder with BeforeAndAfterAll with BeforeAndAfterEach {
  *
  ...
  *    val userDF = sparkSession.read.json(dataDir + "/movielens_users.json")
  *    userDF.write.format("solr").options(Map("zkhost" -> zkhost, "collection" -> "movielens_users", "batch_size" -> "10000")).save
  *
  *    // Read movielens table
  *    val s_movielens_options = Map("zkhost" -> zkhost, "collection" -> "movielens_users", "batch_size" -> "10000")
  *    val s_movielensDF = sparkSession.read.format("solr").options(s_movielens_options).load
  *    println("s_movielensDF: "+s_movielensDF.toString)
  *    s_movielensDF.show()
  *
  ...
  *
  *    // read movies table
  *    val s_movies_options = Map("zkhost" -> zkhost, "collection" -> s"movielens_movies_$uuid", "batch_size" -> "10000")
  *    val s_moviesDF = sparkSession.read.format("solr").options(s_movies_options).load
  *    println("s_moviesDF: "+ s_moviesDF.toString())
  *    s_moviesDF.show()
  *
